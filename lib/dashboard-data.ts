...
export async function getRevenueVsSpendTrend() {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    months.push({ label: start.toLocaleDateString("en-US", { month: "short" }).toUpperCase(), start, end });
  }
  ...
  for (const m of months) {
    const [rev, sp] = await Promise.all([
      ...
      prisma.adSpendDaily.aggregate({ _sum: { spend: true }, where: { date: { gte: m.start, lt: m.end } } }),
    ]);
...
// Ranks clients by ROAS this month
export async function getTopPerformingClients(limit = 5) {
  ...
  const rows = await Promise.all(
    clients.map(async (client) => {
      ...
      return { id: client.id, name: client.name, slug: client.slug, roas: sp > 0 ? rev / sp : 0, conversions: leadCount };
    })
  );
...
export async function getClientDailySpend(clientId: string, days = 10) {
  ...
  const rows = await prisma.adSpendDaily.findMany({ where: { clientId, date: { gte: start } }, orderBy: { date: "asc" } });
  return rows.map((r) => Number(r.spend));
}

// Daily ad spend across the agency (or one client)
...
// Report summary numbers, scoped to one client or the whole agency
